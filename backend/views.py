...
def tuzilma_teg(request, language='uz', tag='-'):
    ...
    employees = [Employee.objects.get(id=i) for i in EMPLOYEE_TAG[tag]]
...
def rektorat(request, language='uz'):
    ...
    employees = [Employee.objects.get(id=6), Employee.objects.get(id=4), Employee.objects.get(id=22), Employee.objects.get(id=32)]
    context['employees'] = employees
    context['request'] = request
    context['language'] = language
    path = request.get_full_path()
